...
std::shared_ptr<IAction> ChangeItemAction::doAction() {
...
void ChangeItemAction::changeItemStuff(std::shared_ptr<ItemInterface> itemInterfacePtr, Map info) {
    for (const auto& pair : info) {
        const auto key = pair.first;
        const auto value = pair.second;
        if (key == "-l" || key == "-t" || key == "-r" || key == "-b") {
            auto bbox = itemInterfacePtr->getBoundingBox();
            ...
            itemInterfacePtr->setBoundingBox(bbox);
        }
        else {
            itemInterfacePtr->setAttribute(key, value);
        }
    }
}

void ChangeItemAction::getItemStuff(std::shared_ptr<ItemInterface> itemInterfacePtr) {
    for (const auto& pair : info_) {
        const Key key = pair.first;
        Value value = pair.second;
        // if (key == "-type") {
            // value = Value(itemInterfacePtr->getType());
        // }
        if (key == "-l" || key == "-t" || key == "-r" || key == "-b") {
            auto bbox = itemInterfacePtr->getBoundingBox();
            if (key == "-l") { value = Value(itemInterfacePtr->getBoundingBox().first.first); }
            if (key == "-t") { value = Value(itemInterfacePtr->getBoundingBox().first.second); }
            if (key == "-r") { value = Value(itemInterfacePtr->getBoundingBox().second.first); }
            if (key == "-b") { value = Value(itemInterfacePtr->getBoundingBox().second.second); }
        }
        else {
            value = itemInterfacePtr->getAttribute(key);
        }
        oldInfo_[key] = value;
    }
}
